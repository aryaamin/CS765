...
class Node:
    ...
    def __init__(self, id):
        ...
        # ledger keeps track of the coin balances 
        # of each node according to the longest chain
        self.ledger = None
        ...
        # an index to keep track of the blocks and transactions generated by the node
        self.block_idx = 0
        self.trxn_idx = 0

        # open file to log events
        os.makedirs("output/logs", exist_ok=True)
        self.log = open("output/logs/log_"+str(self.id)+".txt", "w")

    def log_event(self, string):
        self.log.write(string + "\n")

    def log_blockchain(self):
        self.blockchain.log("output/blockchains/blockchain_"+str(self.id)+".txt")
    ...
    # utility function to find the closest parent of a new block in the longest chain
    ...
    # function to get ledger and pending transactions in another branch 
    ...
    # queues an event that sends the generated transaction to peers 
    ...
    # sends the transaction to peers and queues the next transaction event
    ...
    # adds the transaction to pending transactions and forwards it to peers
    ...
    # starts mining a block and queues an event to send the block to peers
    def generate_block_event(self, curr_time):
        ...
        for t in self.pending_transactions:
            if(ledger_copy[t.payer_id] < t.coins):
                continue
            if t.payer_id is not None:
                ledger_copy[t.payer_id] -= t.coins
                ledger_copy[t.payee_id] += t.coins
        ...
        Tk = np.random.exponential(Node.I/self.hash_power)
        new_event = Event(curr_time + Tk, self, EventType.SEND_BLOCK)
        new_event.block = new_block
        self.simulator.add_event(new_event)
        self.is_mining = True
        pass
    
    # sends the new block to peers if longest hasn't changed and starts mining the next block
    def send_block(self, event, curr_time):
        ...
        # restart mining if there are pending transactions 
        if len(self.pending_transactions) > 0:
            self.is_mining = False
            self.generate_block_event(curr_time)
            
    # checks if the transactions in the block create negative balance and if the mining reward is correct
    ...
    # adds a block to the longest chain if it is valid
    ...
    # adds a block to a chain that is not the longest chain if it is valid
        # return True if successful
    ...
    # receives a block, adds it to blockchain if it is valid and forwards it to peers
